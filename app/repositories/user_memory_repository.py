...
class UserMemoryRepository:
    ...
    async def get_active_facts(self, *, user_id: str) -> list[UserMemory]:
        """Return the user's active facts in stable key order.
        ...
        statement = (
            ...
            .order_by(UserMemory.memory_key.asc(), UserMemory.id.asc())
        )
        try:
            result = await self.db.execute(statement)
        except SQLAlchemyError as exc:
    ...
    async def get_relevant_episodes(
        ...
        *,
        ...
        """Return up to three same-model episodes above a cosine threshold."""
        if not 0 <= min_similarity <= 1:
            raise ValueError("Minimum memory similarity must be between 0 and 1")
        if not 1 <= limit <= 3:
            raise ValueError("Relevant episode limit must be between 1 and 3")
        ...
        statement = (
            ...
            .where(
                UserMemory.user_id == user_id,
                UserMemory.is_active.is_(True),
                UserMemory.memory_type == MEMORY_TYPE_EPISODE,
        ...
        try:
            ...
            await self.db.rollback()
            logger.exception("Relevant episode retrieval failed")
            raise PersistenceError("Could not retrieve user memories") from exc
    ...
    async def create_episode(
        ...
        *,
        ...
        """Stage an independent episodic memory for the user."""
        ...
        memory = UserMemory(
            user_id=user_id,
            memory_type=MEMORY_TYPE_EPISODE,
            memory_key=None,
            content=content,
